...
4. 自动化对比：一次性跑完不同单笔买入股数(300/600/1000)以及不同 TopN(1/3/5/10/15)
   的结果，并生成详细对比报表。
5. 改动说明：现已将买入价格从收盘价改为同日开盘价买入，卖出保持不变（收盘价卖出）。
...
# ==========================================
# 复权因子还原真实价格配置
# Qlib 数据集中的 $open 和 $close 通常为复权价。为了计算真实的资金流，需要还原为真实价格。
# 模式 1: 真实价格 = $open或$close / $factor (最常见，如 Qlib 默认的 Yahoo / Tushare 复权处理)
# 模式 2: 真实价格 = $open或$close * $factor (部分特定格式的数据集)
# 模式 0: $open或$close 本身就是真实不复权价格 (无需转换)
# ==========================================
...
def get_price_info(instruments: list[str], date: str) -> dict[str, dict]:
    ...
    df.columns = ["open", "close", "factor"]
    df = df.reset_index()
    # 只要买卖需要，则必须确保当天开盘和收盘数据均有效
    df = df.dropna(subset=["open", "close"])

    result: dict[str, dict] = {}
    for _, row in df.iterrows():
        model_open = float(row["open"])
        model_close = float(row["close"])
        factor = float(row["factor"]) if not pd.isna(row["factor"]) and float(row["factor"]) != 0 else 1.0

        if PRICE_RESTORE_MODE == 1:
            real_open = model_open / factor
            real_close = model_close / factor
        elif PRICE_RESTORE_MODE == 2:
            real_open = model_open * factor
            real_close = model_close * factor
        else:
            real_open = model_open
            real_close = model_close
...
def select_trade_candidates(df: pd.DataFrame, top_n: int) -> pd.DataFrame:
    ...
    ranked = ranked[ranked["pos_ratio"] > 0.8]
    # ---------------------------------------------
    ...
    for i in range(1, len(predict_dates)):
        ...
        if previous_positions:
            ...
            for pos in previous_positions:
                ...
                if not info or pd.isna(info["real_close"]) or info["real_close"] <= 0:
                    still_hold.append(pos)
                    continue
                
                # 按照收盘价进行卖出结算
                model_sp = info["model_close"]
                sp = info["real_close"]   
                sfactor = info["factor"]  
        ...
        if today_buy_df is not None and not today_buy_df.empty:
            ...
            for inst in buy_instruments:
                ...
                if not info or pd.isna(info["real_open"]) or info["real_open"] <= 0:
                    continue

                # ⭐ 买入逻辑改为同日开盘价买入
                model_bp = info["model_open"]
                bp = info["real_open"]
                bfactor = info["factor"]
                ...
                new_positions.append({
                    "instrument": inst,
                    "buy_real_price": bp,
                    "buy_model_price": model_bp, # 记录模型开盘复权价
...
def main(
    ...
    out: str = "./tests8buy/sim_trade_result.csv",
    detail_out: str = "./tests8buy/sim_trade_detail.csv",
):
