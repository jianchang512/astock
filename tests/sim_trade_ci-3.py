...
def backtest_final(
    ...
    改为 T+3 卖出逻辑：T+1 买入后，持仓天数满 3 天时卖出。
    """
    ...
    for i in range(1, len(predict_dates)):
        ...
        for pos in active_positions:
            pos["days_held"] += 1
            # 持有满 3 个交易日（T+1 买，T+4 此时 days_held=3）
            if pos["days_held"] >= 3:
                sell_candidates.append(pos)
            else:
                still_hold.append(pos)

        if sell_candidates:
            sell_prices = get_close_prices([p["instrument"] for p in sell_candidates], today)
        ...
        # 更新未卖出的股票池（包括还没到期的，以及到期但因停牌没卖掉的）
        active_positions = still_hold
...
def main(
    ...
    for n in top_n_list:
        ...
        result_path = out_path.with_name(f"{out_path.stem}_top{n}-3{out_path.suffix}")
        detail_result_path = detail_path.with_name(f"{detail_path.stem}_top{n}-3{detail_path.suffix}")
    ...
    if compare_rows:
        ...
        compare_path = out_path.with_name(f"{out_path.stem}_compare-3{out_path.suffix}")
